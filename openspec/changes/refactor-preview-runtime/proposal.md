...
- `preview-input-preparation`: Command-side file and runtime-export discovery, validation, normalized export-level preview manifests, generated entries, and lifecycle cleanup before vendor startup.
- `preview-browser-runtime`: A single env entry and HTML document that dispatches the shared component overview, docs, and selected exported demos from `location.hash`, using content-local dynamic import functions and live hash navigation.
...
- Requires deterministic static analysis of supported JavaScript and TypeScript demo-file exports without evaluating framework modules in the Node preparation process.
