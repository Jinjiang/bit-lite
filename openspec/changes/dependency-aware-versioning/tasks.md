...
## 2. Version anchor in the workspace model

- [ ] 2.1 Read an optional `version` field from `.comp.json` in `readWorkspace` and expose it on `WorkspaceComponent`; absent means the component has never been recorded
- [ ] 2.2 Validate the field as a non-empty string and reject malformed values with the existing `.comp.json` diagnostic style
- [ ] 2.3 Confirm `workspace:*` detection for internal dependencies and envs is untouched, and add a regression test asserting the placeholder still drives internal dependency and env resolution
- [ ] 2.4 Add unit tests for present, absent, and malformed `version` fields
...
- [ ] 4.3 Implement `project()` as a pure function taking the working `.comp.json`, the component's `bit-lite.json` entry, and a resolved-version lookup, producing the committed bytes by resolving placeholder dependencies, injecting `env`, and stripping `version`
...
- [ ] 4.6 Add unit tests for version formatting, the reserved-shape predicate, and every projection case: placeholder dependency, local env, external env, no dependencies, and version stripping

## 5. Dependency-ordered snap

- [ ] 5.1 Drive `runSnapCommand` through the shared dependency ordering instead of selection order
- [ ] 5.2 Resolve each component's placeholder dependency and env versions from versions settled earlier in the run, falling back to the store's canonical head refs, never from an on-disk `version` anchor
...
- [ ] 5.7 Write each recorded component's `version` anchor back to its working `.comp.json` only after publication succeeds, preserving the file's existing formatting conventions
- [ ] 5.8 Add integration tests over a real bare store for: ordered recording of a dependency chain, an env recorded before its user, a dependent gaining a new version when only its dependency changed, unchanged detection across repeated snaps, both strictness failures leaving no ref and no file modified, and cycle rejection
...
- [ ] 6.5 Write the tagged component's `version` anchor back after the tag ref is published
...
- [ ] 7.1 Write each local dependency's current version into `createGeneratedPackageManifest` instead of the workspace placeholder, falling back to `0.0.0`
...
- [ ] 8.1 Update demo workspace fixtures and any test fixtures that assert on `.comp.json` shape or on generated manifest versions
