...
- Restrict an assigned component version to exactly `major.minor.patch`. Prereleases and build metadata are refused, which is both simpler to reason about and strictly stronger than reserving the generated identifier shape.
- **BREAKING** Rework `tag` into a multi-component operation: it selects components the way every other workspace command does, processes them in dependency order, and derives a version for each one by incrementing the patch of that component's highest existing version. `--version` becomes optional and is accepted only when the selection resolves to exactly one component.
- Add `--dry-run`, `--json`, and `--message` to both `snap` and `tag`.
...
- `component-version-tags`: Tagging becomes a multi-component, dependency-ordered operation that derives a version per component and may create a snap when the projection changes a component's content; assigned versions are restricted to `major.minor.patch`.
...
- `packages/bit-lite/src/commands/snap.ts` and `tag.ts`: own the workspace dependency graph, drive the topological loop, call the pure projection function, enforce `--filter` strictness, and write the `version` anchors back to `bit-lite.json` in one file update only after the ref transaction succeeds. Both gain `--dry-run`, `--json`, and `--message`, and `tag` gains version derivation over a component's existing tags.
- `packages/bit-lite-history`: gains listing a component's assigned versions so `tag` can derive the next one.
