...
- Require each env package's default `"."` package export to resolve to a JSON `EnvDefinition`. Resolve the exported entry, parse and validate its JSON, and use the npm package name as the env identity and grouping key without executing an env factory.
...
- Updates `bit-lite-vendors`, the `test`, `preview`, and `compile` commands, and preview preparation to preserve selected-env and declaring-env origin separately across inline and worker execution.
