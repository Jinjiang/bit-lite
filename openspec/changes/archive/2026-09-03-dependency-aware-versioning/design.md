...
A version is derived only for a component that has something new. A component whose projected content matches its current snap **and** whose snap already carries a version is skipped: there is nothing to release, so releasing anything would be noise. Without that rule, running `tag` twice in a row assigns a second version to every component, and because a dependent records its dependency's version, each repetition also creates a new snap for every dependent — permanent, immutable history for an operation in which nothing happened. `snap` already worked this way through its tree comparison; `tag` needs the extra condition because an unchanged component that has never been released still has something to release.

The skip propagates correctly on its own. A skipped dependency keeps the version it carries, so a dependent projecting against it sees no change either and is skipped in turn.
