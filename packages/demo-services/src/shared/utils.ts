...
export function relativePath(filePath: string | undefined): string | undefined {
  if (!filePath) {
    return undefined;
  }
...
export function makeLintResult(input: {
  ...
  return {
    ...
    targetFiles: input.targetFiles.map((item) => relativePath(item) ?? item),
...
export async function writeArtifact(vendor: string, fileName: string, code: string): Promise<string> {
  const outputPath = path.join(artifactsRoot, vendor, fileName);
