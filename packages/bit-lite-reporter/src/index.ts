...
export type ReporterServiceTask = {
  listen(listener: (type: string, payload: unknown) => void): () => void;
  abort(): void;
  call(type: string, payload?: unknown): void;
};

export type ReporterWorkspace = {
  groups: Array<{
    envName: string;
    env: {
...
export type ServiceRunReporter = {
  onTask?(task: ReporterServiceTask, context: ServiceRunEventContext): void;
...
export function createDashboardOutputReporter(options: DashboardOutputReporterOptions): DashboardOutputReporter {
  ...
  const tasks = new Map<string, ReporterServiceTask>();
...
export function getServiceVendorLabels(workspace: ReporterWorkspace, serviceName: string) {
